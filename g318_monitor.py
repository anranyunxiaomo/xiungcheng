...
def send_msg(token, content):
    ...
    print("微信全量美食暖心路书推送结果:", res)
    return res

def fetch_30min_realtime_radar():
    """
    30分钟云端高精度巡查 + 互联网高赞餐厅 + 诗意高质感文案雷达引擎
    """
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M")
    
    tomorrow_str = (datetime.now() + timedelta(days=1)).strftime("%m-%d")

    # 包含 2026 互联网高赞餐厅推荐的 9 天全量文案库
    food_poetic_database = {
        "08-01": {
            "title": "成都市 ➔ 康定市",
    ...
    target_date = tomorrow_str if tomorrow_str in food_poetic_database else "08-01"
    data = food_poetic_database[target_date]
...
def push_auto_schedule():
    ...
    if has_status_changed(card_content):
        print("30分钟巡查：更新全量高赞美食与诗意暖心卡片，立即下发微信实时推送！")
        ...
        if res.get("errcode") != 0:
            ...
            tmpl_payload = {
                ...
                "data": {
                    "first": {"value": "🌸 30分钟云端雷达·全量美食与诗意暖心路书推送", "color": "#1890ff"},
                    "keyword1": {"value": "甘孜州气象局/甘孜交警12328/社媒24h实测", "color": "#cf1322"},
                    "keyword2": {"value": "【全量暖心守护】已完成美食/路况/卫生间/防晒/光影最新校对", "color": "#333333"},
